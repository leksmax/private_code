...
#define TRACERT_PING_ONCE_COUNT			3
...
int create_icmp_socket(void)
{
	...
	LOGI("proto ? proto->p_proto : 1 = %d\n", proto ? proto->p_proto : 1);
...
int sk_start_ping(const sk_ping_attribute_t* pPingAttribute, sk_ping_results_t* pPingResults)
{
	...
	//DataBlockSize
	if(pPingAttribute->nDataBlockSize<=0)
		datalen=32;
	else
...
int sk_start_traceroute(const sk_traceroute_attribute_t* pTracertAttribute, sk_traceroute_results_t* pTracertResults)
{
	...
	if (pTracertAttribute->nDataBlockSize <= 0)
	{
	...
	/*
	int k = 0;
	for (k = 0; k < pTracertResults->nNumberOfRouteHops; k++)
	{
		LOGI("wangtonggui test pTracertResults->gHopHost[k].szHost = %s\n", pTracertResults->gHopHost[k].szHost);
	}
	*/
	end_time=gettime_ms ();
	pTracertResults->nResponseTime		= end_time-start_time;
	//pTracertResults->nNumberOfRouteHops	=nTempMaxHopCount;
