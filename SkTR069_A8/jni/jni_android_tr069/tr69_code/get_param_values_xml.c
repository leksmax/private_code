...
int evcpe_get_param_attr_response_to_xml(
		struct evcpe_get_param_attrs_response *method,
		struct evbuffer *buffer)
{
	int rc;
	evcpe_debug(__func__, "marshaling evcpe_get_param_attr_response");
	if ((rc = evcpe_param_attr_list_to_xml(&method->parameter_list,
			"ParameterList", buffer)))
		goto finally;
finally:
	return rc;
