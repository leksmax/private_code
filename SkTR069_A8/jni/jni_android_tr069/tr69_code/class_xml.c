...
{
	int rc = -1;
	struct evcpe_class_parser parser = {0};
	struct evcpe_attr_schema root = {0};
	...
	evcpe_info(__func__ , "android dlopen start\n");
	#ifdef WIN32
	if (!(parser.dynlib = GetModuleHandle(0))) {
		evcpe_error(__func__, "failed to open current process as dynamic lib");
		return -1;
	}
	#else //WIN32
	#ifdef TR069_ANDROID
	if (!(parser.dynlib = dlopen("/system/lib/libsktr069.so", RTLD_LAZY|RTLD_GLOBAL))) 
	{
		evcpe_error(__func__, "failed to open current process as dynamic lib: %s", dlerror());
		LOGI("kangzh android dlopen 1,back\n");
		return -1;
	}
	#else //TR069_ANDROID
...
//解析每一项 ， 将该项放到stack 中， 在 attr 函数中peek 出来然后继续对该elem 操作
int evcpe_class_xml_elm_begin_cb(void *data,
		const char *ns, unsigned nslen, const char *name, unsigned len)
{
	int rc = -1 , extension = 0;
	struct evcpe_class_parser *parser = data;
	struct evcpe_xml_element *parent = NULL , *elm = NULL;
	struct evcpe_attr_schema *schema = NULL , *parent_schema = NULL;
		...
		const char *value, unsigned value_len)
{
	...
	{
		if (schema->type == EVCPE_TYPE_OBJECT || schema->type == EVCPE_TYPE_MULTIPLE) 
		{
			evcpe_error(__func__, "'notification' is not applicable to "
					"type: %d", schema->type);
			rc = EPROTO;
		...
		if (evcpe_strncmp("schema", elm->name, elm->len)) 
		{
			evcpe_error(__func__, "unexpected element: %.*s",elm->len, elm->name);
			rc = EPROTO;
			goto finally;
		}
		...
		#ifdef WIN32
		if (!(schema->getter = (evcpe_attr_getter)GetProcAddress(parser->dynlib ,symbol))) 
		{
			evcpe_error(__func__, "failed to load symbol: %s(errno = %d)", symbol ,GetLastError());
			rc = -1;
			goto finally;
		}
		#else	//WIN32 
		if (!(schema->getter = dlsym(parser->dynlib, symbol)) &&(error = dlerror())) 
		{
	...
	else if (!evcpe_strncmp("setter", name, name_len)) 
	{
		...
		snprintf(symbol, sizeof(symbol), "%.*s", value_len, value);
		#ifdef WIN32
		if (!(schema->setter = (evcpe_attr_setter)GetProcAddress(parser->dynlib ,symbol))) 
		{
			evcpe_error(__func__, "failed to load symbol: %s (errno = %d)", symbol,GetLastError());
			rc = -1;
			goto finally;
		...
		#else //WIN32
		if (!(schema->setter = dlsym(parser->dynlib, symbol)) &&(error = dlerror())) 
		{
			evcpe_error(__func__, "failed to load symbol: %s", error);
			rc = -1;
			goto finally;
   ...
   else if (!evcpe_strncmp("visible", name, name_len)) {
		...
		if (!evcpe_strncmp("true", value, value_len)) 
		{
			schema->visible = EVCPE_PARAM_INFO_VISIBLE;
		} else if (!evcpe_strncmp("false", value, value_len)) {
		    schema->visible = EVCPE_PARAM_INFO_INVISIBLE;
		}else{
            evcpe_error(__func__, "'visible' value must be (true|false)': %.*s",
					value_len, value);
			rc = EPROTO;
			goto finally;
        }
