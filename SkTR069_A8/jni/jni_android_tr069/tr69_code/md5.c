...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void MD5Update (MD5_CTX *context, unsigned char *input, unsigned int inputLen, sk_md5_callback callback)
/* context */
/* input block */
...
/* context */
{
	...
	*/
	...
	MD5Update (context, (unsigned char*) PADDING, padLen, NULL);

	/* Append length (before padding) */
	MD5Update (context, bits, 8, NULL);
 ...
 */
const char* MDString (char *string, unsigned int str_len, sk_md5_callback callback)

{
	...
	int i;
	MD5Init(&context);
	MD5Update(&context, (unsigned char*)string, str_len, callback);
 ...
 */
const char* MDFile (char *filename)

{
	...
	else
	{
		...
		{
			MD5Update (&context, buffer, len, NULL);
		}
		MD5Final (digest, &context);
		fclose (file);
		for (i = 0; i < 16; i++)
...
const char* hmac_md5(char* text, char*  key)
{
	char   digest[16];
	char   output1[32];
	static char output[33] = {""};
	...
	if (key_len > 64)
	{
		...
		MD5Init(&tctx);
		MD5Update(&tctx, (unsigned char*) key, key_len, NULL);
		MD5Final(tk, &tctx);
	...
	*/
	...
	MD5Init(&context);                   /* init context for 1st
                                              * pass */
	MD5Update(&context, k_ipad, 64, NULL);      /* start with inner pad */
	MD5Update(&context, (unsigned char*)text, text_len, NULL); /* then text of datagram
...
*/
	...
	MD5Init(&context);                   /* init context for 2nd
                                              * pass */
	MD5Update(&context, k_opad, 64, NULL);     /* start with outer pad */
	MD5Update(&context, (unsigned char*) digest, 16, NULL);     /* then results of 1st
                                              * hash */
