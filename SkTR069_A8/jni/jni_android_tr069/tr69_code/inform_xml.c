...
int evcpe_inform_to_xml(struct evcpe_inform *method, struct evbuffer *buffer)
{
    ...
    //add by lijingchao
    ev = evcpe_event_list_find(&method->event , "4 VALUE CHANGE");
    if(NULL == ev)
    {
    ...
    else
    {
        ...
        evcpe_info(__func__ , "evcpe_inform_to_xml 4 VALUE CHANGE inform!\n");
        if ((rc = evcpe_param_value_list_to_xml_for_value_change(&method->parameter_list,
			"ParameterList", buffer)))
        {
		    goto finally;
        }
