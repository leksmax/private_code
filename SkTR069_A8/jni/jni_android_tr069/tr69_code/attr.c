...
int evcpe_attr_get(struct evcpe_attr *attr, const char **value, unsigned int *len)
{
	...
	if (attr->schema->type == EVCPE_TYPE_OBJECT || attr->schema->type == EVCPE_TYPE_MULTIPLE) 
	{
		...
		struct evcpe_param_value_list *list)
{
	...
	case EVCPE_TYPE_MULTIPLE:
		...
		break;
	default:
		if(sk_is_node_traceroute(attr->path))
		{
			if ((rc = evcpe_param_value_list_add(list, &param,attr->path, attr->pathlen))) 
			{
		...
		struct evcpe_param_info_list *list, int next_level)
{
	...
	switch (attr->schema->type) {
	case EVCPE_TYPE_OBJECT:
		//printf("[EVCPE_TYPE_OBJECT] attr->schema->name=%s\n",attr->schema->name);

        if(EVCPE_PARAM_INFO_VISIBLE != attr->schema->visible)
        {
			evcpe_debug(__func__ ,"attr->schema->name=%s , visible is false\n",attr->schema->name);
			break;
        }
		...
		TAILQ_FOREACH(schema, &attr->schema->class->attrs, entry) {
			...
			if (next_level) {
				if (schema->type == EVCPE_TYPE_OBJECT ||
                    ...
                    if(EVCPE_PARAM_INFO_VISIBLE == schema->visible)
                    {
    					if ((rc = evcpe_param_info_list_add(list, &param,
		...
		break;
	case EVCPE_TYPE_MULTIPLE:
		//printf("[EVCPE_TYPE_MULTIPLE] attr->schema->name=%s\n",attr->schema->name);
		...
		break;
	default:
		//printf("[default] attr->schema->name=%s\n",attr->schema->name);
		if(EVCPE_PARAM_INFO_VISIBLE == attr->schema->visible)
		{
    		if ((rc = evcpe_param_info_list_add(list, &param,
