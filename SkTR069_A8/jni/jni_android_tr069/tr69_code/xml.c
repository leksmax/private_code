		...
		const char *node, const char *string)
{
	if(string!=NULL)
		...
		return evcpe_add_buffer(buffer, "<%s></%s>\n", node, node);
}
...
int evcpe_xml_add_datetime(struct evbuffer *buffer,
		const char *node,const char *string)
{
	return evcpe_add_buffer(buffer, "<%s xsi:type=\"xsd:dateTime\">%s</%s>\n",
			node, string, node);
}
...
int evcpe_xml_add_str_array(struct evbuffer *buffer,
		const char *node)
{
	return evcpe_add_buffer(buffer, "<%s SOAP-ENC:arrayType=\"xsd:string[0]\"/>\n",
			node);
}
