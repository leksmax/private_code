...
struct evcpe_msg *evcpe_msg_new(void)
{
	...
	memset(msg,0,sizeof(struct evcpe_msg));
	msg->major = 1;
	msg->minor = 0;
	return msg;
}
...
void evcpe_transform_complete_free(struct evcpe_transfer_complete *method)
{
	if (!method) return;
	evcpe_debug(__func__, "destructing evcpe_transform_complete_free");
	free(method);
}

void evcpe_transform_complete_response_free(struct evcpe_transfer_complete_response *method)
{
	if (!method) return;
	evcpe_debug(__func__, "destructing evcpe_transform_complete_free");
	free(method);
...
void evcpe_msg_free(struct evcpe_msg *msg)
{
	...
	//printf("[evcpe_msg_free]msg->type=%d,msg->method_type=%d\n",msg->type,msg->method_type);
	...
	if (msg->data) {
		...
		case EVCPE_MSG_REQUEST:
			...
			case EVCPE_INFORM:
				...
				break;
            case EVCPE_DOWNLOAD:  
                evcpe_download_free(msg->data); //+ by aobai
                break;
            case EVCPE_UPLOAD:
			...
			case EVCPE_TRANSFER_COMPLETE:		//kangzh
				evcpe_transform_complete_free(msg->data);
				break;
			case EVCPE_FACTORY_RESET:
				evcpe_factory_reset_free(msg->data);
				break;
			default:
		...
		case EVCPE_MSG_RESPONSE:
			...
			case EVCPE_SET_PARAMETER_VALUES:
				...
				break;
            case EVCPE_DOWNLOAD:
               	evcpe_download_response_free(msg->data);  //+ by aobai                             	
                break;
            case EVCPE_UPLOAD:
				evcpe_upload_response_free(msg->data);
                break;
          	case EVCPE_TRANSFER_COMPLETE:		//kangzh
				evcpe_transform_complete_response_free(msg->data);                   
			case EVCPE_INFORM:
				evcpe_reboot_response_free(msg->data);
				break;
			case EVCPE_FACTORY_RESET:
				evcpe_factory_reset_response_free(msg->data);				
				break;
			case EVCPE_REBOOT:
