...
{
	struct evcpe_set_param_attrs_response *method;
	evcpe_debug(__func__, "constructing evcpe_set_param_attrs_response");
	if (!(method = calloc(1, sizeof(struct evcpe_set_param_attrs_response)))) {
...
{
	if (!method) return;
	evcpe_debug(__func__, "destructing evcpe_set_param_attrs_response");
