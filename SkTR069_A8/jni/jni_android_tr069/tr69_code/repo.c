...
//			goto finally;
		...
		{
		//	printf("[kangzh 2] evcpe_repo_locate (attr)->path=%s\n\n",(*attr)->path);
		} 
		...
		const char *value, unsigned int len)
{
    ...
    //add by lijingchao
    //date:2014-09-10
    //comment:江苏移动项目，对各个参数设置，进行异常测试。
    //目前针对tr069_model.xml 里面既没有setter ， 也default 情况下，服务器对终端进行设置，终端因为不具备可写属性。报9008错误
    if(/*attr->schema->write != 'W' &&*/ NULL == attr->schema->setter && NULL == attr->schema->value)
    {
        
        evcpe_error(__func__, "failed to set value by setter: %s => %.*s , because param is not writeable!",
				attr->path, len, value);
        rc = EVCPE_CPE_NON_WRITABLE_PARAM;
        goto finally;    
    }
    //add end
    #endif
		...
		char *value, unsigned int len)
{
	...
	if (ptrlen >= len) 
	{
		evcpe_error(__func__, "value exceeds string limit,name:%s : %d >= %d",
				name,ptrlen, len);
		rc = EOVERFLOW;
		goto finally;
	}
...
int evcpe_repo_add_obj(struct evcpe_repo *repo, const char *name,unsigned int *index)
{
	int rc;
	struct evcpe_obj *obj;
	struct evcpe_attr *attr;
...
int evcpe_repo_del_obj(struct evcpe_repo *repo, const char *name)
{
	...
	if (!attr || attr->schema->type != EVCPE_TYPE_MULTIPLE) {
		evcpe_error(__func__, "not an multiple object attribute: %s", name);
		rc = EINVAL;
		goto finally;
	}
	if ((rc = evcpe_attr_del_obj(attr, index-1))) {
		...
		struct evcpe_param_value_list *list)
{
	...
	TAILQ_FOREACH(schema, &obj->class->attrs, entry)   //先找到每一个节点，再找到每个节点的属性
	{
        ...
        evcpe_debug(__func__ ,"evcpe_repo_to_inform_param_value_list  object name:%s,type:%d\n",(schema->name),schema->type);
		...
		default:
			///////////////////////////////////////////////////////////////
			if((sk_get_node_inform(attr->path)) || (schema->inform)|| sk_is_node_logmsg(attr->path) )
			{
		...
		//sunjian:
		if(sk_is_node_lalarmmsg(attr->path) )
		{
			if ((rc = evcpe_attr_to_param_value_list(attr, list))) 
			{
				evcpe_error(__func__,"failed to add param to value list");
		...
		if(sk_is_node_xingneng(attr->path))
		{
			if ((rc = evcpe_attr_to_param_value_list(attr, list))) 
			{
				evcpe_error(__func__,"failed to add param to value list");
...
int evcpe_repo_to_inform(struct evcpe_repo *repo, struct evcpe_inform *inform)
{
	...
	if ((rc = evcpe_repo_to_inform_param_value_list(repo->root,	&inform->parameter_list))) 
	{
		...
		struct evcpe_param_info_list *list, int next_level)
{
	...
	if (*name == '\0') 
	{
        if(EVCPE_PARAM_INFO_VISIBLE == attr->schema->visible)
        {
